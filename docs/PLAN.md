...
- [~] **Phase 4.5c — PRIORITY: multi-entry actors + event-queue semantics (corrects the single-entry model in the current implementation)**
  ...
  - Progress
    ...
    - [ ] step 2b (PRIORITY, persistent queues) — swap broadcast for okm-backed persistent partitions per the PERSISTENT QUEUE RULING above (`[mq-data][event][part_id][time]` + `[mq-cursor][event][part_id][actor]{cursor}`); backpressure becomes visible backlog, re-activation replays unconsumed events, skip-to-now relief valve; realm event path rewrite (queue read = range scan + cursor advance, subscribe = register cursor at now)
