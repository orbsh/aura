...
- [ ] Phase 2.5 — Script-actor ctx bridge: host functions exposing ctx.state / ctx.invoke into carrier scripts (steel host fns, python via pyo3 closures, nu via plugin/extern); async suspension of script calls. Required before script actors can hold state.
