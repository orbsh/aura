...
- [~] **Phase 4.9 — Type-scoped actor storage (PRIORITY, ADR-0026, docs/adr/0026-type-scoped-actor-storage.md en+zh)**
  ...
  - Work items
    ...
    - [ ] wasm schema path: okm compiles INTO the module — script implements VirtualStorage over aura_host emit imports, runs the real Collection API in-module (static derives at wasm build time; no dynamic schema form for storage)
