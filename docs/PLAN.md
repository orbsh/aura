...
- [~] **Phase 4.9 — Type-scoped actor storage (PRIORITY, ADR-0026, docs/adr/0026-type-scoped-actor-storage.md en+zh)**
  ...
  - Work items
    ...
    - [~] op set narrowing: protocol types landed (`aura-actor/src/store_emit.rs` — StoreOp/StoreOpKind, JSON wire, Collection-semantic layer); put/get/delete_document + SCAN (schema-declared AccessMethod) + REDUCE GET (count/high_water/lowwater presets) EXECUTE through okm-dynamic DynamicCollection (`realm/src/store_exec.rs`); field-level ops pending (dynamic-segment bridge); host-language reduce logic objects register through bindings, not schema data. No bypass guard: the developer has full control, primitive misuse is self-sabotage
    ...
    - [ ] InstanceState retirement: state.rs document model + StateDocumentStore seam removed, replaced by typed-ns tables; engine tests migrate off field-level ctx.store.get/set (reduce watermarks already migrated to okm ADR-0023 HighWater presets — b926bdd)
