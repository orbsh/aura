...
- [ ] Phase 6.6 — Storage Actor (ADR-0010): host `#[kv_storage]` executor instances
  ...
  - Value representation (ADR-0018, 2026-09-20 — accepted, to implement): delete `StoreAsVirtual` (the base64-in-JSON adapter over the raw byte trait), bind the mq tables to `FjallStore` (same fjall database, its own keyspace), represent actor state as one document per instance, and keep JSON only at the actor-API seam through a single `serde_json::Value ↔ DynamicValue` module. Clean break: existing mq/state bytes are discarded, no migration.
