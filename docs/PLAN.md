...
- [ ] Phase 6.6 — Storage Actor (ADR-0010): host `#[kv_storage]` executor instances
  ...
  - connection-plane half LANDED (2026-09-20): `Realm::kv_pending` + `kv_round_trip` / `kv_round_trip_within` push one raw op frame to a probe's declared executor and resolve the correlated answer — `Frame::Kv` returns the executor's bytes unparsed, `Frame::KvRefused { executor, kv_id, reason }` returns the stated reason (a refusal is an ANSWER: neither a dropped frame nor an empty Ok that hides the cause; the probe gateway's reader also stops treating it as an unknown frame, which used to bail and tear the connection down). The round-trip deadline is the caller's policy (`KV_ROUND_TRIP_TIMEOUT`, the hot-call magnitude); a timeout withdraws the correlation so a late answer is discarded rather than delivered to a caller that gave up. Test `crates/engine/tests/kv_round_trip.rs` (answer / refusal / silence / unconnected alias). Remaining: the `#[kv_storage]` backend that consumes this (a `VirtualStorage` impl over the remote round trip) and the executor-side operator surface.
