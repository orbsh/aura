...
- Now (design accepted, not implemented): `InstanceState` document model, the `StateDocumentStore` seam, and the field-level ctx bridge host fns are scheduled for replacement by the typed-ns surface. Implementation sequence lands as PLAN Phase 4.9 (op-set narrowing, registry→ns allocation, InstanceState retirement, per-language schema declaration, ctx.interface_schema read). Terminology: okm's collection/document replaces the older table/row phrasing throughout this ADR and its PLAN entry.
