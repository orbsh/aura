...
How to model a domain onto Aura actors: pick the instance key first (the standing-ownership criterion), then define events and handlers, keep state instance-scoped, and collaborate across instances through events and projections. This guide is the application-side norm; for mechanisms see [partitioning.md](partitioning-en.md), [realm.md](realm.md), [actor-api.md](actor-api.md).

> **Languages:** [中文](modeling.md) · English (this file)

## Step 1: pick the instance key for each actor

An actor type = one piece of code + the swarm of instances expanded by its instance key. The criterion for choosing a key is the **instance's standing ownership**, not a field the request happens to carry:
...
```
Group-chat channel actor (instance key c1)
