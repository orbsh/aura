# Aura 建模指南（Booths Guide）

如何把一个业务域建模为 Aura 摊位：先选分区键（恒等归属判据），再定事件与 handler，状态只属于实例，跨实例协作走事件与投影。本文是应用侧的使用规范；机制细节见 [partitioning.md](partitioning.md)、[realm.md](realm.md)、[booth-api.md](booth-api.md)。

> **Languages:** 中文（本文）· [English](modeling-en.md)

## 第一步：为每种摊位选分区键

一个摊位类型 = 一份代码 + 按 instance key 展开的实例群。选键的判据是**实例的恒等归属**，不是请求碰巧携带的字段：
...
事件是 Booth 之间的唯一协作通道（场域 pub/sub，见 booth-api.md）：
...
状态存储在类型层（ADR-0026 §3）：每个摊位类型占一个真实 okm ns，类型声明自己的 collections（schema 随 interface_schema 上传持久化），实例是类型 ns 内的 document；handler 经 `ctx.store.emit(op)` 读写。建模约束：

- **collection 里只放本类型恒等归属的数据**：购物车类型放条目 document；群聊类型放成员表、最近消息。把别的类型的数据复制进来会产生第二真相源。
- **跨类型读取不可表达**：这是设计而非限制——存储寻址在注册时绑定类型 ns，跨类型的数据协作走事件（对方摊位处理后 emit 结果），或直接调用取回。
...
## 第四步：跨实例聚合走投影摊位跨分区查询（统计部门所有用户的购物车、某用户在哪些群）不能 JOIN 也不该全扫——用**投影摊位**：

- 一个普通的事件接收摊位，按自己的维度分区（如按 dept_id 或 user_id），`@on` 监听上游实例 emit 的事件，持续把事件聚合进自己的 collections。**同类型实例间的聚合不再需要投影**（ADR-0026 §3：类型 ns 内的普通 scan/reduce）；投影只剩跨类型预计算一个用途。
...
实例休眠不丢数据（状态落盘、事件留在队列），驻留省的是**下次激活的成本**：重新拉起 VM/probe、重载脚本、重建内存态。`idle_ttl` 按摊位类型声明（`with_idle_ttl` builder，声明于 `interface_schema` 的 lifecycle 段），不驻留是默认——留不留按激活成本与使用模式的对比判断：
...
直写路径上的真实成本不是 fjall 本身，是两处次要开销：

- **JSON ↔ DynamicValue 转换**（value.rs seam）：每字段一次。tick 内合并整帧写一次（一条快照），不要 per-field 100 次小写。
- **文档粒度的语义错配**：okm 文档模型的 per-field write 为低频字段设计；「一帧一个整体快照」要合并成单次 put，保持快照语义。

超过这个规模（tick 更密、字段更多、房间数更大），才切到 **session 内存**形态：
...
游戏服务端三层（prism 连接/aura 逻辑/probe 执行）与房间制的对应、以及帧驱动 vs 消息驱动的缺口，见 PLAN Phase 8 与 partitioning.md §5。

## 外发投递：场域的边界在哪里

场域的事件投递覆盖 Booth ↔ Booth；把消息送到场域外的连接（用户的 WS 长连接）是**外发投递**，由外层的连接平面（prism，Phase 8）承担——aura 不知道 WS 的存在：

```
群聊 channel booth (分区键 c1)
...
- **booth 只 emit，不寻址连接**：`emit` 的载荷带目标 user_id，WS 连接的注册表在连接平面——摊位层永远不需要知道「谁在线、连接在哪」，这同样是「分区键答路由、参数答身份」的边界应用。
- **outbound 桥是普通订阅者**：单例 wildcard 消费 `outbound.*`，无特殊通道——与投影摊位同构，只是它把「投」作为副作用而不是写状态。
- **反向（入站）已解**：客户端消息经 prism 按事件名投进场域（`emit("channel_msg", ...)`，分区键 channel_id 自带在消息里），见第一步的群聊例子。
- **离线用户**：emit 落在持久队列，per-user 的会话/离线逻辑由一个按 user_id 分区的普通摊位（或连接平面的离线队列）承接——是应用建模决策，不是引擎机制。
...
| 需求 | 形态 |
|---|---|
| 用户自己的数据 | 按 user_id 分区的摊位，身份读 `ctx.self_id.key` |
...
| 每帧高频状态（游戏房间） | 默认 fjall 直写（整帧合并单次 put）；预算超限才切 session 内存 + 事件回放恢复 |
